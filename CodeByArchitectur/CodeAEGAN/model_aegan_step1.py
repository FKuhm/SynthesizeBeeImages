...
import os
from modules_aegan_step1 import *
...
def trainGAN(
    ...
    decoder = Decoder().to(device).apply(weights_init)

    # Loss function: L1 Loss for EncDec and Wasserstein-Loss for GenDisc:
    enc_dec_loss = nn.L1Loss()

    # Specify optimizers:
    optimizer_enc = torch.optim.Adam(params=encoder.parameters(), lr=lr_enc, betas=(0.5, 0.9))
    optimizer_dec = torch.optim.Adam(params=decoder.parameters(), lr=lr_dec, betas=(0.5, 0.9))
    ...
    for u in range(0,int(np.ceil(10000/batch_size))):
        ...
        images = decoder(generator(torch.randn(size = (batch_size, latent_dim, 1, 1), device = device)))
    ...
    with torch.no_grad():
        ...
        generated_images = decoder(generator(noise_vecs))
    ...
    for epoch in range(0, number_epochs_enc_dec):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            res = decoder(encoder(data))
            loss = enc_dec_loss(res,data)
            loss.backward()
            optimizer_enc.step()
            optimizer_dec.step()

            enc_dec_error.append(loss)

        wandb.log({"loss_encdec": loss.item()})

        
        print("############################")
        print("Epoch %i completed" % (epoch))
        print("Loss of Encoder/Decoder: %f" %
            (loss.item()))
        print("############################")
    ...
    for epoch in range(0, number_epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            # Push to GPU:
            data = autograd.Variable(encoder(data.to(device)))
            ...
            epsilon = torch.rand(size = (x_tilde.size()[0], data.size()[1], data.size()[2],data.size()[3]), device = device)
        ...
        if (epoch < 10 and epoch%1 == 0) or (epoch >= 10 and epoch%10 == 0):
            ...
            torch.save(generator.state_dict(), os.path.join(image_output_dir, wandb.run.name, "generator_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth"))
            torch.save(discriminator.state_dict(), os.path.join(image_output_dir, wandb.run.name, "discriminator_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth"))
            torch.save(encoder.state_dict(), os.path.join(image_output_dir, wandb.run.name, "encoder_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth"))
            torch.save(decoder.state_dict(), os.path.join(image_output_dir, wandb.run.name, "decoder_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth"))

        # Log model at the end of run:
        if epoch == number_epochs-1:
            wandb.save(os.getcwd() + "/generator_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth", policy = 'now')
            wandb.save(os.getcwd() + "/discriminator_AEGAN_090622_run_" + wandb.run.name + "_epoch_" + str(epoch) + ".pth", policy = 'now')
...
def trainGAN_sweep(config, wandb, dir_path):
    ...
    print("\n Start of Training\n")
    print(config)
    trainGAN(config.lr_gen, config.lr_disc, config.latent_dim, config.epochs, config.batch_size, wandb, dir_path, True, config.n_critic, "/home/ws/uwhbq/1_AEGAN/generated_bee_images",\
        lr_dec=config.lr_dec, lr_enc=config.lr_enc, number_epochs_enc_dec= config.number_epochs_enc_dec)
